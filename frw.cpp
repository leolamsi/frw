/*
syntax: frw.exe [q, N, L, t_max, outn, dir name, file name]
*/
#include <iostream>
#include <math.h>
#include <time.h>
...
void next_hop() {	// choose particle k to hop
	...
	int ran_ptcl = (int)(Ran2() * N_hop_ok); //ran ptcl
...
void out_to_file() {
    if (t >= (double)dt * t_cnt /*&& t_cnt < 1000*/) { //output to file
	t_cnt += 1;
	int p_xpos[N_MAX];
	for (int i = 0; i < N; ++i) {
...
#if pair_deviation
    int count = 0;
    for (int i = 0; i < 10000; ++i) { //simulate 10000 pairs
	if (i >= count * 10) { //print progress
	    cout << i/100. << "%" << endl;
	    ++count;
	}
	t = 0;
